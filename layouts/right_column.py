...
from components.graphs import (
    ...
    create_customers_stacked_graph  # Nova função a ser implementada
)
...
def create_right_column(dfs, total_hours, total_hours_ytd):
    ...
    return html.Div(
        className='column',
        children=[
            # Seção de Utilização por Tracks - MODIFICADA: Limitada aos 8 principais, mantida na coluna central
            create_section_container([
                ...
                html.Div(
                    ...
                    children=[
                        create_graph_section(
                            'monthly-tracks-graph',
                            create_tracks_graph(dfs['tracks'], height=320, max_items=8)
                        )
                    ]
                )
            ]),

            # Seção de Utilização por Áreas - MODIFICADA: Mantida na coluna central com design simplificado
            create_section_container([
                ...
                html.Div(
                    ...
                    children=[
                        create_graph_section(
                            'monthly-areas-graph',
                            create_areas_graph(dfs['areas'], height=320)
                        )
                    ]
                )
            ]),

            # Seção de Utilização por Clientes - MODIFICADA: Novo modelo de gráfico, mantida na coluna central
            create_section_container([
                ...
                html.Div(
                    ...
                    children=[
                        create_graph_section(
                            'ytd-customers-graph',
                            create_customers_stacked_graph(dfs['customers_ytd'], height=320)
                        )
                    ]
                )
            ], margin_bottom='0px')
